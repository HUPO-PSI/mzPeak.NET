...
public class IsolationWindowBuilder : ParamVisitorCollection, IArrowBuilder<List<Param>>
{
    ...
    {
        var cols = ColumnMappingsFromVisitors();
        foreach (var c in cols)
        {
            c.Path = ["isolation_window", .. c.Path];
        }
        return cols;
    }
}
...
public class PrecursorBuilder : IArrowBuilder<(ulong, ulong?, string?, List<Param>, List<Param>)>
{
    ...
    public List<ColumnMapping> ColumnMappings()
    {
        ...
        cols.AddRange(Activation.ColumnMappingsFromVisitors().Select(e =>
        {
            e.Path = ["activation", ..e.Path];
            return e;
        }));
        return cols;
    }
...
public class ScanBuilder : ParamVisitorCollection, IArrowBuilder<(ulong, uint?, double?, string?, ulong?, string?, List<Param>, List<List<Param>>)>
{
    ...
    public List<ColumnMapping> ColumnMappings()
    {
        var entries = ColumnMappingsFromVisitors();
        entries.AddRange(ScanWindowListBuilder.ColumnMappings());
        return entries;
    }

    public ScanBuilder() : base(new()
    {
        new CustomBuilderFromParam(ScanAttribute.ScanStartTime.CURIE(), "scan start time", new DoubleType(), Unit.Minute.CURIE()),
        new CustomBuilderFromParam(ScanAttribute.FilterString.CURIE(), "filter string", new StringType()),
        new CustomBuilderFromParam(ScanAttribute.PresetScanConfiguration.CURIE(), "preset scan configuration", new Int64Type()),
        new CustomBuilderFromParam(ScanAttribute.IonInjectionTime.CURIE(), "ion injection time", new DoubleType(), Unit.Millisecond.CURIE()),
    })
    {
...
public class ScanWindowBuilder : ParamVisitorCollection, IArrowBuilder<List<Param>>
{
    ...
    public List<ColumnMapping> ColumnMappings()
    {
        var visitors = ColumnMappingsFromVisitors();
        foreach(var v in visitors)
        {
            v.Path = ["scan_window", ..v.Path];
        }
        return visitors;
    }
}
...
public class ScanWindowListBuilder : IArrowBuilder<List<List<Param>>>
{
    ...
    public List<ColumnMapping> ColumnMappings() => ValueBuilder.ColumnMappings();
...
public class SelectedIonBuilder : ParamVisitorCollection, IArrowBuilder<(ulong, ulong?, List<Param>)>
{
    ...
    public SelectedIonBuilder() : base(new()
        {
            new CustomBuilderFromParam("MS:1000744", "selected ion mz", new DoubleType(), "MS:1000040"),
            new CustomBuilderFromParam("MS:1000042", "peak intensity", new FloatType(), "MS:1000131"),
            new CustomBuilderFromParam("MS:1000041", "charge state", new Int32Type())
        })
    {
        SourceIndex = new();
        PrecursorIndex = new();
...
public class WavelengthSpectrumBuilder : ParamVisitorCollection, IArrowBuilder<(ulong, string, double, string?, List<Param>, EntryDerivedMetadata?)>
{
    ...
    public WavelengthSpectrumBuilder() : base(new()
    {
        ...
        // Optional spectrum properties (commonly present)
        new CustomBuilderFromParam(SpectrumProperties.NumberOfDataPoints.CURIE(), "number of data points", new UInt64Type()),
        new CustomBuilderFromParam("MS:1003812", "lambda max", new DoubleType(), Unit.Nanometer.CURIE()),
